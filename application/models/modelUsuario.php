...
class ModelUsuario extends CI_Model {
    ...
    function cadastrar($nome,$email, $telefone, $renasem, $cpfcnpj, $CEP, $endereco, $num, $bairro, $cidade, $estado, $senha, $produtor ,$tecnico)
	{
	   	$query = $this->db->query('INSERT INTO `sementeseviveiros`.`usuarios` (`email`, `nome`, `telefone`, `senha`, `CEP`, `endereco`, `numero`, `bairro`, `cidade`, `estado`, `cpfcnpj`, `renasem`, `admin`, `produtor`, `tecnico`) VALUES (\''.$email.'\', \''.$nome.'\',\''.$telefone.'\', \''.$senha.'\', \''.$CEP.'\', \''.$endereco.'\', \''.$num.'\', \''.$bairro.'\', \''.$cidade.'\', \''.$estado.'\', \''.$cpfcnpj.'\', \''.$renasem.'\', \'0\', \''.$produtor.'\',\''.$tecnico.'\');');
	}
}